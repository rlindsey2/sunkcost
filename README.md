...
Memory fit: `weights_gb + kv_cache(context) ≤ usable_memory_gb`. KV cache is derived per layer from `n_kv_heads × head_dim × 2 bytes × 2`, with sliding-window layers capped at their window and linear-attention layers ignored. Usable memory on Macs follows the macOS default GPU wired limit (two-thirds at ≤32 GiB, 75% above). On graphics cards it is the VRAM less 1 GB, the margin llama.cpp's automatic fitting leaves free per device. Graphics cards carry `price_scope: "card_only"`: they are priced as the card alone and labelled that way wherever the price appears. Each card's estimate factors (`estimate_efficiency_dense`, `estimate_efficiency_moe`) are calibrated on its own measured rows, because mixture-of-experts models reach only about a third to a half of the bandwidth ceiling on CUDA.
...
- **The context it was measured at** (`tctx`). A speed measured at a short context is scaled to the context you're asking about, the same way our measurements are; left blank, it is taken as measured at the slider's context. The note warns when a speed is more than 1.5× what the memory bandwidth allows, which almost always means a prompt-processing figure.
- **Key-value cache type** (`kv`: `f16`, `q8_0`, `q4_0`), under the assumptions. llama.cpp block sizes (34 and 18 bytes per 32 values) shrink the cache, which changes both what fits and how fast long contexts run. See `defaults.kv_cache`.
...
Everything served is a static file, so traffic is free and unmetered on Cloudflare's free plan. The one exception is the card functions above. Drawing a card takes roughly 100 ms of CPU, past the free plan's 10 ms per request, so they need Workers Paid ($5 a month). They are off unless the GitHub repository variable `DYNAMIC_CARDS` is `on`: `scripts/prepare-functions.ts` then routes `/s/*` and `/card/*` to functions in `dist/_routes.json`; off, only `/card/*` is routed, nothing links there, and share pages stay plain files. The limits that matter are 20,000 files per deployment (the build writes about 4,050) and 25 MiB per file. `public/_headers` sets cache lifetimes. DNS is on Cloudflare, where the domain is registered, and a redirect rule sends `www` to the apex.
